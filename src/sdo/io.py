...
basedir = '/gpfs/gpfs_gl4_16mb/b9p111/fdl_sw/SDOML'
filename_template = \
'{0:s}/{1:04d}/{2:02d}/{3:02d}/{4:s}{5:04d}{6:02d}{7:02d}_{8:02d}{9:02d}_{10:s}.npz'

def sdo_read(year, month, day, hour, min, instr='AIA', channel='0094',
    ...
    min - between 0 and 59 (note AIA data is at 6 min cadence, 
...
def sdo_find(year, month, day, hour, min, instrs=['AIA','AIA','HMI'], 
             channels=['0171','0193','bx'], subsample=1, basedir=basedir, 
             return_images=False):
    
    """
    Purpose: Find filenames of multiple channels of the SDOML dataset with the same timestamp. 
    ...
    ind = 0
    for ch in channels:
        files.append(filename_template.format(
    ...
    if files_exist:
        if (not return_images):
            ...
            img = zeros(shape=(int(512/subsample),int(512/subsample),
